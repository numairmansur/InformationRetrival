...
public class ListIntersection {
   ...
   */
  PostingList readPostingList(String fileName, int numRepeats,
   ...
   */
  public PostingList intersectBinarySearch(PostingList listA,
    PostingList listB) {
    ArrayList<Integer> ids = new ArrayList<Integer>();
    ArrayList<Integer> scores = new ArrayList<Integer>();
   ...
   */
  public PostingList intersectGallopSearch(PostingList listA,
    PostingList listB) {
    ArrayList<Integer> ids = new ArrayList<Integer>();
    ArrayList<Integer> scores = new ArrayList<Integer>();
    ...
    for (int i = 0; i < listA.ids.length; i++) {
      int j = exponentialSearch(listB.ids, listA.ids[i],
        lastIntersectedValueInListB, listB.ids.length - 1);
      if (j != -1) {
        ids.add(listA.ids[i]);
        scores.add(listA.scores[i] + listB.scores[j]);
    ...
    for (int i = 0; i < listA.ids.length; i++) {
      while (listA.ids[i] > listB.ids[pointer]) {
        ...
        if (pointer >= listB.ids.length) {
          pointer = listB.ids.length - 1; // If pointer goes out of bound,
                                          // set it to the length of list B
        }
      ...
      // Binary search is giving better results than exponential
      int j = binarySearch(listB.ids, listA.ids[i], lowerBound, pointer);
      if (j != -1) {
        ids.add(listA.ids[i]);
        scores.add(listA.scores[i] + listB.scores[j]);
        lowerBound = j;
